          ...
          <aside>
            ...
            {selectedTypes.includes('mock-test') ? (
              <form className="material-panel material-panel--mock-test" onSubmit={previewMockTest}>
                ...
                {!mockTestPreview ? <button className="btn btn-primary" disabled={mockTestLoading}>{mockTestLoading ? 'Validating…' : 'Validate Mock Test for selected course'}</button> : <div className="mock-test-preview"><b>Total questions: {mockTestPreview.totalRows} · Accepted: {mockTestPreview.validRows} · Duplicate questions ignored: {mockTestPreview.duplicateRows || 0} · Rejected: {mockTestPreview.invalidRows}</b>{mockTestPreview.invalidRows ? <ul>{mockTestPreview.rows.filter((row) => !row.valid && !row.skipped).slice(0, 5).map((row) => <li key={row.rowNumber}>Row {row.rowNumber}: {row.validationErrors.join(', ')}</li>)}</ul> : null}<button className="btn btn-primary" disabled={mockTestLoading} onClick={importMockTest} type="button">{mockTestLoading ? 'Importing questions…' : 'Save accepted questions'}</button></div>}
              </form>
            ) : null}
