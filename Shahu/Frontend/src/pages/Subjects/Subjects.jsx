      ...
      {mockTestSubject ? (
        <div className="login-overlay">
          <form className="student-form validity-form mock-test-upload-modal" onSubmit={previewMockTest}>
            ...
            {!mockTest.preview ? <button className="btn btn-primary" disabled={mockTest.loading}>{mockTest.loading ? 'Checking file…' : 'Validate questions'}</button> : <div className="mock-test-preview"><b>{mockTest.preview.validRows} ready to import · {mockTest.preview.invalidRows} rejected{mockTest.preview.duplicateRows ? ` · ${mockTest.preview.duplicateRows} duplicates skipped` : ''}</b>{mockTest.preview.invalidRows ? <ul>{mockTest.preview.rows.filter((row) => !row.valid && row.validationErrors?.length).slice(0, 5).map((row) => <li key={row.rowNumber}>Row {row.rowNumber}: {row.validationErrors.join(', ')}</li>)}</ul> : null}<button className="btn btn-primary" onClick={importMockTest} type="button">Import valid questions</button></div>}
          </form>
        </div>
      ) : null}
    </section>
